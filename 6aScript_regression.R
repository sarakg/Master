...
stargazer(logit1.1, logit2.1, logit3.1, 
          ...
          dep.var.labels = "Autocratization Dummy LDI", 
          covariate.labels = c("Mean World Change LDI lag", 
                               "Mean Region Change LDI lag", 
                               "Mean Neighbor Change LDI lag"), 
          no.space = TRUE, 
...
(exp(17.114)-1)*100 
exp(-2.175) # Intercept, 0.113
(exp(-2.175)-1)*100

# Region
exp(5.363) # 213 times as high odds
(exp(5.363)-1)*100 # 21236% higher odds 
...
(exp(-1.920)-1)*100 # -85%
exp(-1.183)
(exp(-1.183)-1)*100 # -69% odds for autocratization

## Ln Population
exp(0.184) # 0.275
(exp(0.184)-1)*100 # 20.2% odds for autocratization
exp(-0.274) # 0.760
(exp(-0.274)-1)*100 # -24% lower odds for autocratization

## Coup d'etat
exp(1.667) # 5.29
(exp(1.667)-1)*100 # 429% odds for autocratization
# Coups rather than not have 5.29 times higher odds for autocratization, controlled
# for the other variables in the model. 

## Civil War
exp(0.774) # 2.16
(exp(0.774)-1)*100 # 116.8%
exp(0.474) # 1.6
(exp(0.474)-1)*100 # 60.6%

## Education
exp(-0.084) # 0.91
(exp(-0.084)-1)*100 # -8% lower odds
